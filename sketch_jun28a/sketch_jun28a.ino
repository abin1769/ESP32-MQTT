...
// ==========================================
// 1. KONFIGURASI WIFI, OTA & MQTT
// ==========================================
const char* ssid = "Aulia Wulandari"; // [cite: 18]
const char* password = "FIDELYAAFRA"; // [cite: 18, 19]

// --- Konfigurasi Web OTA ---
const char *host = "esp32-webupdate"; // [cite: 1]
const char *authUser = "F1E324033";   // [cite: 3]
const char *authPass = "0052738487";  // [cite: 3]
...
void callback(char* topic, byte* payload, unsigned int length) {
  String message = ""; // [cite: 38]
  for (unsigned int i = 0; i < length; i++) { // [cite: 39]
    message += (char)payload[i]; // [cite: 39]
  }
  logToWeb("MQTT Rcv [" + String(topic) + "]: " + message); // [cite: 40]

  if (String(topic) == String(topic_rgb)) { // [cite: 41]
    int r = 0, g = 0, b = 0; // [cite: 41]
    if (sscanf(message.c_str(), "%d,%d,%d", &r, &g, &b) == 3) { // [cite: 42]
      setRGBColor(r, g, b); // [cite: 42]
      logToWeb("RGB Updated -> R:" + String(r) + " G:" + String(g) + " B:" + String(b)); // [cite: 43]
    } else {
      logToWeb("Gagal parsing format warna RGB!"); // [cite: 44]
    }
  }

  if (String(topic) == String(topic_cmd)) { // [cite: 45]
    if (message == "reboot") {
      logToWeb("Sistem direboot remote dari Web..."); // [cite: 45]
      delay(1000); // [cite: 46]
      ESP.restart(); // [cite: 46]
    } else if (message == "ping") {
      logToWeb("Pong! ESP32-S3 is alive."); // [cite: 46]
    } else if (message == "led_off") { // [cite: 47]
      setRGBColor(0, 0, 0); // [cite: 47]
      logToWeb("Semua warna LED dimatikan."); // [cite: 47]
    } else {
      logToWeb("Command tidak dikenali: " + message); // [cite: 48]
    }
  }
}
...
void setup(void) {
  ...
  server.on("/", HTTP_GET, []() { // [cite: 7]
    if (!server.authenticate(authUser, authPass)) return server.requestAuthentication(); // [cite: 7]
  ...
  server.on("/update", HTTP_POST, []() { // [cite: 8]
    ...
    if (upload.status == UPLOAD_FILE_START) { // [cite: 9]
      ...
      authenticated = server.authenticate(authUser, authPass); // [cite: 9]
